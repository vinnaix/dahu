...
public class SolrCloudIndexerProcessor extends AbstractProcessor {
    ...
    public SolrCloudIndexerProcessor(Level _level, Component _component){
        super(_level, _component);

    }

    protected static void setIsCommitting(boolean _isCommitting){
        SolrCloudIndexerProcessor.isCommittingFlag = _isCommitting;
    }
    ...
    private String deleteDoc(String _id){
        if (this.solr != null) {
            try {
                ...
                // Solr UpdateResponse is not fully implemented yet so no point in using it to test for success
                //https://lucene.apache.org/solr/8_0_0/solr-solrj/org/apache/solr/client/solrj/SolrClient.html
//                System.out.println("Delete response as String = " + response.toString());

                return "success"; // assume it's all good unless we  get an exception
            } catch (IOException ioe) {
